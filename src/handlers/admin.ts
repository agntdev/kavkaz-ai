...
composer.callbackQuery("admin:ack", async (ctx) => { await ctx.answerCallbackQuery(); if (!(await requireOwner(ctx as any))) return; await ctx.reply("Отчёты отмечены как просмотренные.", { reply_markup: inlineKeyboard([[inlineButton("В главное меню", "menu:main")]]) }); });
